...
The [presentation layer](/trainer/presentation) picks `Match.Won` up from there, plays the post-game takeover, and emits `Event.Match.PostGameSettled` once the player has chosen a follow-up action. Embedders should listen for that rather than `Match.End` when they need to know the post-game flow is finished.
...
| `Event.Match.PostGameSettled` | The post-game flow settled (takeover closed, or none was shown) |
