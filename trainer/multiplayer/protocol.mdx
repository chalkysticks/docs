...
| `snapshot-full` | host | Full-state recovery response. Bypasses the staleness gate and advances the high-water mark, with two receiver-side refusals: it is ignored while the receiver's own shot is still in flight (a forced apply would clobber a mid-shot table with the peer's stale view of it), and ignored entirely in durable async-paced matches, where the versioned API row outranks any unversioned peer snapshot. | full state plus `requestReason` |
...
| `shot-in-progress` | active shooter | Answer to a mid-shot join: the shooter announces that its own shot is already rolling (the joiner subscribed too late to see the `shot:taken` packet, and the canonical snapshot is still pre-shot). The joiner holds its hydration cover until the resolving packet or a canonical convergence read lands. Carries no state version and never participates in durable authority. Sent by `Facade.broadcastShotInProgress()` when the opponent (re)joins while `localShotMotionComplete` is false. | `actorSeat`, `matchId` |
...
`snapshot-full` bypasses the staleness check but still advances the mark so a late older resolution cannot rewind past the snapshot. Two receiver-side guards temper "always applies": a peer snapshot is ignored while the receiver's own shot is in flight (the shooter's own resolved state and the versioned API row remain the recovery paths — suppression is scoped to the local shot-in-flight window only, so spectator and idle-watcher recovery keep applying snapshots normally), and in durable async-paced matches an unversioned peer snapshot is refused outright in favor of persisted authority.
...
4. Both clients hold input for the duration of the handshake through the capability Gate — the live-rack dimension of the `System/Gate` `InputAuthority` state machine — and release it from the adopted turn once the ACK lands. The hold is bounded: when the host exhausts its 8 `rack-start` re-sends without a `rack-ack`, it treats the silent peer as absent and releases the rack hold itself (the versioned API snapshot is what catches the peer up on return), so an unanswered handshake can no longer strand input.
