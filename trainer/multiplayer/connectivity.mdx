...
sequenceDiagram
    ...
    Client->>Ably: presence enter with name, avatar, role, userId
    Client->>Ably: query current presence set
...
**Presence** is how both connects and disconnects are detected. Entering a room enters Ably presence with `avatarUrl`, `name`, `role` (`player` or `spectator`), and `userId`, then **enumerates the current member set** (`getPresenceMembers`, backed by sdk-extras `AblyChannel.getPresence()`) so an opponent who arrived first is detected immediately instead of waiting for the `match:create` / `match:join` message handshake — which remains as identity/repair. A presence `enter` subscription catches late arrivals the same way. Evaluation filters spectators, the local client's own entry (by `userId`), and userId-less members, and is additive-only: a failed or empty query never clears the opponent-connected flag, so presence enumeration is evidence of an opponent, never proof of absence. All detectors deduplicate through one guard, so `Event.Multiplayer.OpponentJoined` fires exactly once per connection with the winning detector as its `reason` (`'presence'`, `'match-create'`, or `'match-join'`).

When a peer's presence *leave* fires, the room controller dispatches `Event.Multiplayer.OpponentLeft` locally — disconnect detection is presence-based, not a wire message. Spectator departures are ignored, and each client unsubscribes from presence-leave before leaving so its own departure never fires its own handler. `Game.ts` mirrors both events into the reactive `game/isOpponentPresent` store flag, which drives the lower-third connectivity dot and the shell's join/leave toasts.
